      ...
      try {
        ...
        if (data.type === 'SIMPLE_MANUAL_POSITION') {
          ...
          if (!isPlaying) {
            ...
            if (data.sizePreset !== undefined) {
              setSizePreset(data.sizePreset)
              addDebugLog(`📐 Size preset: ${data.sizePreset}`)
              console.log('🔍 Ausspielung received sizePreset:', data.sizePreset)
            }
          } else {
        ...
        <div className="bg-gray-900 p-2 flex items-center justify-between border-b border-gray-700">
          ...
          <div className="flex items-center gap-3">
            ...
            <div className="text-xs text-gray-400">
              {isPlaying ? '▶️ PLAYING' : '⏸️ PAUSED'} | Speed: {speed} | Size: {sizePreset} | H: {flipHorizontal ? 'ON' : 'OFF'} | V: {flipVertical ? 'ON' : 'OFF'}
            </div>
          </div>
        </div>
      )}
